...
namespace Telerik.StringExtensions
{
    ...
    public static class StringExtensions
    {
        ...
        public static string ToMd5Hash(this string input)
        {
            ...
            StringBuilder collectBytes = new StringBuilder();
            for (int i = 0; i < byteArray.Length; i++)
            {
                collectBytes.Append(byteArray[i].ToString("x2"));
            }   
                      
            return collectBytes.ToString();
        }

        /// <summary>
        /// Check if a string contains true value(ex: true, ok, yes, 1).
        /// </summary>
        /// <param name="input">String that should be checked if it has true value or not.</param>
        /// <returns>Returns boolean result if the input has true value (ex: true, ok, yes, 1).</returns>
