...
class ReplaceStartAndFinishWholeWord
{
    static void Main()
    {
        GenerateLargeFile(); //this metod generate large file ~100 MB
        string fileContent = File.ReadAllText("../../start.txt");
        File.WriteAllText("../../finish.txt", Regex.Replace(fileContent, "start ", "finish"));
